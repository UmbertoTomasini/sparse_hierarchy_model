...
from hierarchical_s0_bs import HierarchicalDataset
...
def train(args, net0, criterion):
    ...
    file_paths = [st+'/dataHier_n_%d_m_%d_nc_%d_L_%d_s0_%d_idx_%d.pt'%(args.num_features,args.m,args.num_classes,args.num_layers,args.s0,idx_bs) for idx_bs in range(num_batches)]
...
def test(args, net, criterion, print_flag=True):
    ...
    with torch.no_grad():
        ...
        if args.s0==0:
            st = 'dataset_creating/storing_all0_s0_0'
        file_paths = [st+'/dataHier_n_%d_m_%d_nc_%d_L_%d_s0_%d_idx_%d.pt'%(args.num_features,args.m,args.num_classes,args.num_layers,args.s0,idx_bs) for idx_bs in range(a0,a1)]
        dataset = MyDataset(file_paths)
