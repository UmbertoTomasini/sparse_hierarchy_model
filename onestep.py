...
def train(args, net0, criterion):
    ...
    if args.s==2:
        ...
        file_paths = [st+'/dataHier_n_%d_m_%d_nc_%d_L_%d_s0_%d_idx_%d_seed_%d.pt'%(args.num_features,args.m,args.num_classes,args.num_layers,args.s0,idx_bs,args.seed_trainset) for idx_bs in range(num_batches)]
    else:
        st = 'dataset_creating/storing_all0_ss_max'
        file_paths = [st+'/dataHier_n_%d_m_%d_nc_%d_L_%d_s0_%d_s_%d_idx_%d_seed_%d.pt'%(args.num_features,args.m,args.num_classes,args.num_layers,args.s0,args.s,idx_bs,args.seed_trainset) for idx_bs in range(num_batches)]
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):          #, (inputs, targets) in enumerate(trainloader):
            ...
            loss = criterion(outputs, targets)*(targets.size(0)/args.ptr)
...
def main():
    ...
    parser.add_argument("--sharings", nargs='+', type=str, help="which layers do implement (s_0+1) sharing")
    ...
    print(args.sharings) ##sharings check
