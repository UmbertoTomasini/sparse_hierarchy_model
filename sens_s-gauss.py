...
#xx_new = torch.tensor([87,440,640,740,840,940,1040,1340,2279,3923])
#len_xx = len(xx_new) 
for (idx_p,ptrc) in enumerate(xx_new):
    ...
    if idx_p==0:
        ...
        mat_all_gauss  = torch.zeros((len_xx,num_layers ,1))
        #mat_all_pos = torch.zeros((len_xx,num_layers ,L))
        #mat_terr = torch.zeros(len_xx)    
    
    if net=='cnn2' or net=='lcn':

        mat = gauss_state2permutation_stability(state, args,s,bs)    
        ...
        for (idxnode,node) in enumerate(nodes):
            ...
            #print(idxnode,node)
            mat_all_gauss[idx_p,idxnode,:] = mat[node]

            #mat, norms  = pos_state2permutation_stability(state, args,s,bs)  
            #mat_all_pos[idx_p,idxnode,:] = mat[node]
        #mat_terr[idx_p] = (100-tmp['best']["acc"])/100
        
torch.save(mat_all_gauss,'mats/mat_gauss_L_%d_n_%d_m_%d_s0_%d_s_%d_bs_%d_%s.pt'%(L,n,m,s0,s,bs, net))
#torch.save(mat_all_sem,'mats/mat_sem_L_%d_n_%d_m_%d_s0_%d_s_%d_bs_%d_%s_FineTunedP2.pt'%(L,n,m,s0,s,bs, net))
#torch.save(mat_all_pos,'mats/mat_pos_L_%d_n_%d_m_%d_s0_%d_s_%d_bs_%d_%s_FineTunedP2.pt'%(L,n,m,s0,s,bs, net))
#torch.save(xx_new,'mats/xx_L_%d_n_%d_m_%d_s0_%d_s_%d_bs_%d_%s_FineTunedP2.pt'%(L,n,m,s0,s,bs, net))
