...
s6=str(    '#SBATCH --time 1:00:00 \n')
s7 = str('#SBATCH --mem 12G \n')
        ...
        for s0 in s0s:
            for s in sss:
                ...
                for n in ns:
                    ...
                    #m = int(n**(s-1))
                    #int(0.5*n**(s-1))
                        basename = "send_sens_"+net+"_n_"+str(n)+"_m_"+str(m)+"_L_"+str(L)+"_s0_"+str(s0)+"_s_"+str(s)
                        s1=str( '#SBATCH --job-name '+ basename + " \n")
                        s2=str(  '#SBATCH --out '+ basename + '.out'+ " \n")
                            
                        aa = str("#!/bin/bash \n")+str0+s00+s1+s2+s3+s4+s5+s6+s7+"python3 sens_s.py --device "+str(device)+ " --net "+str(net)+" --s "+str(s)+" --n "+str(n)+" --m "+str(m)+" --s0 "+str(s0)+" --L "+str(L)+" --bs "+str(bs)
