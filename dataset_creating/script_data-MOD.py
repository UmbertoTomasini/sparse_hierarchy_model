    ...
    for s0 in s0s:
        for s in sss:
            ...
            for n in ns:
                ...
                basename = "data_max_n_"+str(n)+"_m_"+str(m)+"_L_"+str(L)+"_s0_"+str(s0)+"_s_"+str(s)+"-MOD"
                s1=str( '#SBATCH --job-name '+ basename + " \n")
                s2=str(  '#SBATCH --out '+ basename + '.out'+ " \n")
                        
                aa = str("#!/bin/bash \n")+str0+s00+s1+s2+s3+s4+s5+s6+s7+"python3 create_dataset-MOD.py --device "+str(device)+" --s "+str(s)+" --n "+str(n)+" --m "+str(m)+" --nc "+str(n)+" --s0 "+str(s0)+" --L "+str(L)+" --output "+str(basename)+".npy"
